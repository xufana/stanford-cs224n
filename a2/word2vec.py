...
def naiveSoftmaxLossAndGradient(
    ...
    y_hat = softmax(np.dot(centerWordVec, outsideVectors.T)) # P(O|C) -- softmax
    loss = - np.log(y_hat[outsideWordIdx])

    delta = y_hat.copy()
    delta[outsideWordIdx] -= 1 # y_hat - y (y - OHE vector)
...
def negSamplingLossAndGradient(
    ...
    uo_vc = np.dot(u_o.T, centerWordVec)
    u_k = outsideVectors[negSampleWordIndices]
    uk_vc = np.dot(u_k, centerWordVec)

    loss = - np.log(sigmoid(uo_vc)) - np.sum(np.log(sigmoid(-uk_vc)))
    gradCenterVec = -(1 - sigmoid(uo_vc)) * u_o + np.sum(np.expand_dims(1 - sigmoid(-uk_vc), axis=1) * u_k, axis=0)
    ...
    for i, v in enumerate(negSampleWordIndices):
        gradOutsideVecs[v] += (1 - sigmoid( - uk_vc)[i]) * centerWordVec
